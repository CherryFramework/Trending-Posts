<?php
/**
 * Init callbacks for additional meta.
 *
...
// If class 'TM_Trending_Posts_Callback_Views' not exists.
if ( ! class_exists( 'TM_Trending_Posts_Callback_Views' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class TM_Trending_Posts_Callback_Views {
		 ...
		 * @var   string
		 */
		public $meta_key = 'tm_trend_views';
		...
		public function __construct() {
			...
			// Hooks a function on to display post `Views` meta.
			add_action( 'tm_trending_posts_display_views', array( $this, 'the_views' ) );
			add_filter( 'tm_trending_posts_return_views', array( $this, 'get_the_views' ) );

			// Handlers for AJAX request.
			add_action( 'wp_ajax_tm_trend_posts_check_views', array( $this, 'ajax_cache_fix' ) );
			add_action( 'wp_ajax_nopriv_tm_trend_posts_check_views', array( $this, 'ajax_cache_fix' ) );
		}
		...
		public function get_views_html( $post_id ) {
			...
			$format = apply_filters(
				'tm_trending_posts_views_format',
				'<span class="meta-rank-views-count" data-id="%2$s">%1$s</span>',
				$views, $post_id
			);
		...
		public function save_views() {
			...
			setcookie( $this->meta_key, maybe_serialize( $views ), time() + TM_TREND_POSTS_CACHE_TIME, COOKIEPATH, COOKIE_DOMAIN );
	...
	TM_Trending_Posts_Callback_Views::get_instance();
