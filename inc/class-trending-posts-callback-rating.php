...
// If class 'TM_Trending_Posts_Callback_Rating' not exists.
if ( ! class_exists( 'TM_Trending_Posts_Callback_Rating' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class TM_Trending_Posts_Callback_Rating {
		 ...
		 * @var   string
		 */
		public $meta_key = 'tm_trend_rating';
		...
		public function __construct() {
			// Hooks a function on to display post `Rating` meta.
			add_action( 'tm_trending_posts_display_rating', array( $this, 'the_rating' ) );
			add_filter( 'tm_trending_posts_return_rating', array( $this, 'get_the_rating' ) );

			// Handlers for AJAX request.
			add_action( 'wp_ajax_tm_trending_posts_handle_rating',        array( $this, 'ajax_handle' ) );
			add_action( 'wp_ajax_nopriv_tm_trending_posts_handle_rating', array( $this, 'ajax_handle' ) );
			add_action( 'wp_ajax_tm_trend_posts_check_rating',            array( $this, 'ajax_cache_fix' ) );
			add_action( 'wp_ajax_nopriv_tm_trend_posts_check_rating',     array( $this, 'ajax_cache_fix' ) );
		}
		...
		public function get_the_rating( $args = array() ) {
			...
			return '<div class="meta-rank-rating">' . $result . '</div>';
		}
		...
		public function get_rating_html( $args = array() ) {
			 ...
			 */
			$defaults = apply_filters( 'tm_trending_posts_rating_defauls', array(
				'post_id'     => get_the_ID(),
				'format'      => esc_html__( '%1$s (%2$s. Average %3$s of %4$s)', 'trending-posts' ),
				'is_disabled' => false,
			) );
			 ...
			 * @param array $args
			 */
			$args = apply_filters( 'tm_trending_posts_rating_args', $args );
			 ...
			 * @since 1.0.0
			 */
			do_action( 'tm_trending_posts_assets' );
			...
			$_votes = sprintf( _n( '%s vote', '%s votes', $rating_meta['votes'], 'trending-posts' ), $rating_meta['votes'] );
			$votes  = '<span class="rating-votes">' . $_votes . '</span>';
			$rating = '<span class="rating-val">' . $rating_meta['rate'] . '</span>';

			$star_rating = $this->get_stars( $rating_meta['rate'], $rating_meta['total'], $args );

			$format = apply_filters(
				'tm_trending_posts_rating_format',
				$args['format'],
		...
		public function get_stars( $current = 0, $total = 5, $args ) {
			...
			return sprintf(
				'<div class="star-rating%3$s" data-post="%2$s" data-format="%4$s">%1$s</div>',
		...
		public function ajax_handle() {
			...
			if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'tm_trend_posts' ) ) {
				die();
			}
			...
			setcookie( $this->meta_key, maybe_serialize( $rates ), time() + TM_TREND_POSTS_CACHE_TIME, COOKIEPATH, COOKIE_DOMAIN );
			...
			do_action( 'tm_trending_posts_rating_ajax_handle', $post_id );
	...
	TM_Trending_Posts_Callback_Rating::get_instance();
}
